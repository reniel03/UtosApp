...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($current['status'] === 'ongoing' || $current['status'] === 'rejected') {
        echo json_encode(['success' => false, 'message' => 'Task status already: ' . ($current['status'] === 'ongoing' ? 'In Progress' : 'Rejected')]);
        exit();
    }
    
    // Update the student's task status from pending to ongoing (in progress)
    $update_stmt = $conn->prepare('UPDATE student_todos SET status = "ongoing" WHERE task_id = ? AND student_email = ?');
    $update_stmt->bind_param('is', $task_id, $student_email);
    
    if ($update_stmt->execute()) {
        // Auto-reject all other pending students for this task
        $reject_others_stmt = $conn->prepare('UPDATE student_todos SET status = "rejected" WHERE task_id = ? AND student_email != ? AND status = "pending"');
        $reject_others_stmt->bind_param('is', $task_id, $student_email);
        $reject_others_stmt->execute();
        $reject_others_stmt->close();
        
        echo json_encode(['success' => true, 'message' => 'Task approved successfully! Other pending applications have been rejected.']);
    } else {
