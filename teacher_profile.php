...
$teacher_name = trim(($_SESSION['first_name'] ?? '') . ' ' . ($_SESSION['middle_name'] ? $_SESSION['middle_name'] . ' ' : '') . ($_SESSION['last_name'] ?? ''));
$teacher_photo = $_SESSION['photo'] ?? null;
$teacher_email_display = $_SESSION['email'] ?? '';
$teacher_department = $_SESSION['department'] ?? 'Department';
$teacher_gender = $_SESSION['gender'] ?? '';

// DEBUG: Log what we're getting
error_log("DEBUG: teacher_gender = [" . $teacher_gender . "]");
error_log("DEBUG: Session keys: " . implode(", ", array_keys($_SESSION)));

// If gender is not in session, fetch it directly from database
if (empty($teacher_gender)) {
    error_log("DEBUG: Gender empty, fetching from database...");
    $stmt = $conn->prepare("SELECT gender FROM teachers WHERE email = ?");
    $stmt->bind_param('s', $teacher_email);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($row = $result->fetch_assoc()) {
        $teacher_gender = $row['gender'] ?? '';
        error_log("DEBUG: Fetched from DB: gender = [" . $teacher_gender . "]");
        // Also update the session
        $_SESSION['gender'] = $teacher_gender;
    }
    $stmt->close();
}

// Check if photo exists or use placeholder
$has_photo = !empty($teacher_photo) && file_exists($teacher_photo);
...
<body>
    ...
    <div class="page-wrapper">
        ...
        <div class="profile-section">
            <?php if ($has_photo): ?>
                <img src="<?php echo htmlspecialchars($teacher_photo); ?>" alt="Profile" class="profile-avatar" title="Click to change profile picture">
            <?php else: ?>
            ...
            <div class="profile-email"><?php echo htmlspecialchars($teacher_email_display); ?></div>
        ...
        <div class="menu-container">
            ...
            <div class="menu-item" onclick="handleMenuClick('edit_profile')">
                <div class="menu-item-left">
                    <div class="menu-item-icon">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#ff0000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 24px; height: 24px;">
                            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                            <circle cx="12" cy="7" r="4"></circle>
                        </svg>
                    </div>
            ...
            <div class="menu-item" onclick="handleLogout()" style="border-left-color: #ff0000;">
                <div class="menu-item-left">
                    <div class="menu-item-icon" style="background: rgba(255, 0, 0, 0.1);">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#ff0000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 24px; height: 24px;">
                    ...
                    <div class="menu-item-text">
                        <div class="menu-item-title" style="color: #ff0000;">Logout</div>
            ...
            <form class="edit-profile-form" id="editProfileForm" onsubmit="handleProfileUpdate(event)">
                ...
                <div class="profile-edit-section">
                    <div class="profile-pic-wrapper">
                        <?php if ($has_photo): ?>
                            <img src="<?php echo htmlspecialchars($teacher_photo); ?>" alt="Profile" id="profilePicPreview" class="profile-pic-edit">
                        <?php else: ?>
                ...
                <div class="form-field-group">
                    ...
                    <input type="text" id="editName" name="name" class="form-input" value="<?php echo htmlspecialchars($teacher_name); ?>" required>
                </div>
                ...
                <div class="form-field-group">
                    ...
                    <select id="editGender" name="gender" class="form-input">
                        ...
                        <option value="male" <?php echo (strtolower($teacher_gender) === 'male') ? 'selected' : ''; ?>>Male</option>
                        <option value="female" <?php echo (strtolower($teacher_gender) === 'female') ? 'selected' : ''; ?>>Female</option>
                    </select>
                </div>
                ...
                <div class="form-field-group">
                    ...
                    <input type="email" id="editEmail" name="email" class="form-input" value="<?php echo htmlspecialchars($teacher_email_display); ?>" required>
                </div>
    ...
    <nav class="bottom-nav">
        ...
        <a href="teacher_profile.php" class="bottom-nav-item active" title="Account">
            <span class="bottom-nav-icon">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                    <circle cx="12" cy="7" r="4"></circle>
    ...
    <script>
        // Debug gender value
        const genderValue = <?php echo json_encode($teacher_gender); ?>;
        console.log('Teacher Gender Value from PHP:', genderValue);
        
        // Initialize gender dropdown when page loads
        window.addEventListener('DOMContentLoaded', function() {
            const genderSelect = document.getElementById('editGender');
            if (genderSelect && genderValue) {
                console.log('Setting gender dropdown to:', genderValue);
                genderSelect.value = genderValue;
                console.log('Dropdown value is now:', genderSelect.value);
            }
        });
        ...
        function openEditProfile() {
            ...
            if (modal) {
                ...
                // Ensure gender value is set when modal opens
                const genderSelect = document.getElementById('editGender');
                if (genderSelect && genderValue) {
                    genderSelect.value = genderValue;
                    console.log('Modal opened - Gender set to:', genderSelect.value);
                }
            }
        }
        ...
        function handlePasswordChange(event) {
                    ...
                    html: `
                        <div style="text-align: center; padding: 0;">
                            ...
                            <p style="
                                ...
                                color: #666;
                                line-height: 1.5;
                            ">New password must be at least 6 characters long!</p>
                        </div>
                    `,
                    confirmButtonColor: '#ff0000',
                    confirmButtonText: 'Okay',
            ...
            // Send data to server
            fetch('change_teacher_password.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
                    ...
                    html: `
                        <div style="text-align: center; padding: 0;">
                            ...
                            <p style="
                                ...
                                color: #666;
                                line-height: 1.5;
                            ">An error occurred: ${error.message}</p>
                        </div>
                    `,
                    confirmButtonColor: '#ff0000',
                    confirmButtonText: 'Okay',
